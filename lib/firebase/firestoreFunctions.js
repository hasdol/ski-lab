...
export const addUserSkis = async (userId, skiData) => {
  try {
    const enriched = {
      ...skiData,
      keywords_en: buildKeywords(skiData, 'en'),
      keywords_no: buildKeywords(skiData, 'no'),
      dateAdded: serverTimestamp(),          // (optional) nice for default sorting
    };
...
export const addTestResult = async (userId, tournamentData, additionalData) => {
  try {
    ...
    const fullData = {
      ...base,
      keywords_en: buildKeywords(base, 'en'),
      keywords_no: buildKeywords(base, 'no'),   // 🔍 keyword array for search
...
export async function updateTestResultBothPlaces(userId, testId, updatedData, sharedEvents = []) {
  // 🔄 regenerate keyword arrays before writing
  const enriched = {
    ...updatedData,
    keywords_en: buildKeywords(updatedData, 'en'),
    keywords_no: buildKeywords(updatedData, 'no'),
  };

  // 1️⃣  update private document
  const userDocRef = doc(db, `users/${userId}/testResults/${testId}`);
  await updateDoc(userDocRef, enriched);

  // 2️⃣  update every live event copy the user kept checked
  await Promise.all(
