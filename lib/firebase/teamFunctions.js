...
/**
 * Team inventory: test skis
 * Stored in: teams/{teamId}/testSkis/{skiId}
 */
export async function addTeamTestSki(teamId, { serialNumber, grind = '' }) {
  if (!teamId) throw new Error('addTeamTestSki: Missing teamId');
  if (!serialNumber || typeof serialNumber !== 'string') throw new Error('Serial number is required');
  const col = collection(db, 'teams', teamId, 'testSkis');
  const payload = {
    serialNumber: serialNumber.trim(),
    grind: typeof grind === 'string' ? grind.trim() : '',
...
export async function updateTeamTestSki(teamId, skiId, { serialNumber, grind = '' }) {
  if (!teamId || !skiId) throw new Error('updateTeamTestSki: Missing teamId/skiId');
  if (!serialNumber || typeof serialNumber !== 'string') throw new Error('Serial number is required');
  const ref = doc(db, 'teams', teamId, 'testSkis', skiId);
  await updateDoc(ref, {
    serialNumber: serialNumber.trim(),
    grind: typeof grind === 'string' ? grind.trim() : '',
    updatedAt: serverTimestamp(),
...
export async function deleteTeamTestSki(teamId, skiId) {
  if (!teamId || !skiId) throw new Error('deleteTeamTestSki: Missing teamId/skiId');
  await deleteDoc(doc(db, 'teams', teamId, 'testSkis', skiId));
}

/**
 * Team inventory: products
 * Stored in: teams/{teamId}/products/{productId}
 */
export async function addTeamProduct(teamId, { brand, name }) {
  if (!teamId) throw new Error('addTeamProduct: Missing teamId');
  if (!brand || typeof brand !== 'string') throw new Error('Brand is required');
  if (!name || typeof name !== 'string') throw new Error('Name is required');
  const col = collection(db, 'teams', teamId, 'products');
  const payload = {
    brand: brand.trim(),
    name: name.trim(),
    createdAt: serverTimestamp(),
...
export async function updateTeamProduct(teamId, productId, { brand, name }) {
  if (!teamId || !productId) throw new Error('updateTeamProduct: Missing teamId/productId');
  if (!brand || typeof brand !== 'string') throw new Error('Brand is required');
  if (!name || typeof name !== 'string') throw new Error('Name is required');
  const ref = doc(db, 'teams', teamId, 'products', productId);
  await updateDoc(ref, {
    brand: brand.trim(),
    name: name.trim(),
    updatedAt: serverTimestamp(),
...
export async function deleteTeamProduct(teamId, productId) {
  if (!teamId || !productId) throw new Error('deleteTeamProduct: Missing teamId/productId');
  await deleteDoc(doc(db, 'teams', teamId, 'products', productId));
}

/**
 * Event-scoped product test results (not saved under users/{uid}/testResults).
 * Stored in: teams/{teamId}/events/{eventId}/productTests/{testId}
 */
export async function addEventProductTestResult(teamId, eventId, payload) {
  if (!teamId || !eventId) throw new Error('addEventProductTestResult: Missing teamId/eventId');
  const col = collection(db, 'teams', teamId, 'events', eventId, 'productTests');
  const data = {
    ...payload,
    createdBy: auth?.currentUser?.uid || null,
...
export async function updateEventProductTestResult(teamId, eventId, testId, patch) {
  if (!teamId || !eventId || !testId) throw new Error('updateEventProductTestResult: Missing ids');
  if (!patch || typeof patch !== 'object') throw new Error('updateEventProductTestResult: Missing patch');

  const ref = doc(db, 'teams', teamId, 'events', eventId, 'productTests', testId);
  await updateDoc(ref, {
    ...patch,
    updatedAt: serverTimestamp(),
    updatedBy: auth?.currentUser?.uid || null,
...
export async function deleteEventProductTestResult(teamId, eventId, testId) {
  if (!teamId || !eventId || !testId) throw new Error('deleteEventProductTestResult: Missing ids');
  await deleteDoc(doc(db, 'teams', teamId, 'events', eventId, 'productTests', testId));
}
